...
def get_data(filters):
	...
	query = (
		...
		.select(
			Complaint.risk_level.as_("risk_level"),
	...
	for row in rows:
		...
		data.append(
			{
				"risk_level": row.risk_level or _("Not Set"),
...
def apply_optional_filters(query, Complaint, Item, ItemMaster, filters):
	...
	if filters.get("complaint_type"):
		...
		matching = (
			frappe.qb.from_(ComplaintType)
			.select(ComplaintType.parent)
			.where(ComplaintType.parenttype == "Complaint")
			.where(ComplaintType.reference == filters.complaint_type)
		)
		query = query.where(Complaint.name.isin(matching))
...
def get_chart(data):
	return {
		"data": {
			"labels": [row["risk_level"] for row in data],
			"datasets": [
				{
					"name": _("Total Amount"),
