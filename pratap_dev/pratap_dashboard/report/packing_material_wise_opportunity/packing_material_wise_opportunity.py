...
def get_columns():
	return [
		...
		},
		{
			...
			"label": _("Packing Material"),
			"fieldtype": "Link",
			"options": "Item",
			"width": 220,
		},
		{
			"fieldname": "packing_material_name",
			"label": _("Packing Material Name"),
			"fieldtype": "Data",
			"width": 240,
		},
		{
...
def get_data(filters):
	...
	query = (
		...
		.select(
			Item.item_code.as_("packing_material"),
			Item.item_name.as_("packing_material_name"),
		...
		data.append(
			{
				...
				"packing_material_name": row.packing_material_name or row.packing_material,
...
def get_chart(data):
	return {
		"data": {
			"labels": [row["packing_material_name"] for row in data],
			"datasets": [
				{
