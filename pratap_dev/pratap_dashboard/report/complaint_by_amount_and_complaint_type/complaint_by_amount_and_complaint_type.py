...
def get_data(filters):
	...
	ComplaintType = frappe.qb.DocType("Complaint Type MultiSelect")
	...
	query = (
		...
		.left_join(ComplaintType)
		.on(
			(ComplaintType.parent == Complaint.name)
			& (ComplaintType.parenttype == "Complaint")
			& (ComplaintType.parentfield == "complaint_type")
		)
		.select(
			ComplaintType.reference.as_("complaint_type"),
		...
		.groupby(ComplaintType.reference)
		.orderby(total_amount, order=frappe.qb.desc)
	)
	query = apply_optional_filters(query, Complaint, Item, ItemMaster, ComplaintType, filters)
	...
	for row in rows:
		...
		data.append(
			{
				"complaint_type": row.complaint_type or _("Not Set"),
...
def apply_optional_filters(query, Complaint, Item, ItemMaster, ComplaintType, filters):
	field_map = {
		...
		"complaint_type": ComplaintType.reference,
...
def get_chart(data):
	return {
		"data": {
			"labels": [row["complaint_type"] for row in data],
			"datasets": [
				{
					"name": _("Total Amount"),
